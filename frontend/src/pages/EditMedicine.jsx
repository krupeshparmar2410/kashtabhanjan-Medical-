...
const EditMedicine = () => {
  ...
  // Load active agencies and medicine details on mount
  useEffect(() => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setFormData({
            ...
            discountAllowed: med.discountAllowed || 0,
            ...
            hsnCode: med.hsnCode || '',
            barcode: med.barcode || '',
            expiryAlertDays: med.expiryAlertDays || 90,
            trackBatches: med.trackBatches !== undefined ? med.trackBatches : true,
            allowPurchase: med.allowPurchase !== undefined ? med.allowPurchase : true,
            allowSale: med.allowSale !== undefined ? med.allowSale : true,
            notes: med.notes || '',
            status: med.status || 'Active',
            isBlocked: med.isBlocked !== undefined ? med.isBlocked : false
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await medicineAPI.updateMedicine(id, formData);
      if (data.success) {
        showToast('Medicine record updated successfully!', 'success');
      ...
      const serverMsg = err.response?.data?.message || 'Server error updating medicine record';
  ...
  if (loadingDetails) {
    return (
      <div className="flex-center" style={{ minHeight: '60vh', flexDirection: 'column', gap: '16px' }}>
        ...
        <p style={{ color: '#64748B', fontSize: '14px' }}>Loading medicine details...</p>
      </div>
    );
  }
      ...
      <form className="form-card" onSubmit={handleSubmit}>
              ...
              <div className="checkbox-row" style={{ marginTop: '22px' }}>
                <label className="form-checkbox-wrapper" style={{ borderColor: formData.isBlocked ? 'var(--error-color)' : 'var(--border-color)', backgroundColor: formData.isBlocked ? 'var(--error-bg)' : 'transparent' }}>
                  <input
                    ...
                    onChange={handleChange}
                  />
                  <span style={{ color: formData.isBlocked ? 'var(--error-color)' : 'inherit' }}>Block Transactions</span>
                </label>
              </div>
            </div>
        ...
        <div className="form-actions-row">
          ...
          <button 
            ...
            disabled={submitting}
          >
            {submitting ? 'Saving...' : 'Update Medicine Record'}
          </button>
        </div>
